...
from config import ProfilePaths, BROWSER_STATE_DIR, STATE_FILE, AUTH_INFO_FILE, DATA_DIR
...
class AuthManager:
    ...
    - Session restoration
    - Multi-profile support (--profile flag)
    """

    def __init__(self, profile_name: str = "default"):
        """Initialize the authentication manager"""
        self.profile_paths = ProfilePaths(profile_name)

        # Use paths from the profile
        self.state_file = self.profile_paths.state_file
        self.auth_info_file = self.profile_paths.auth_info_file
        self.browser_state_dir = self.profile_paths.browser_state_dir
        self.browser_profile_dir = self.profile_paths.browser_profile_dir
    ...
    def setup_auth(self, headless: bool = False, timeout_minutes: int = 10) -> bool:
        ...
        """
        print(f"🔐 Starting authentication setup for profile: {self.profile_paths.name}...")
        ...
        try:
            ...
            # Launch using factory with profile paths
    ...
    def clear_auth(self) -> bool:
        ...
        """
        print(f"🗑️ Clearing authentication data for profile: {self.profile_paths.name}...")
    ...
    def validate_auth(self) -> bool:
        ...
        print(f"🔍 Validating authentication for profile: {self.profile_paths.name}...")
...
def main():
    ...
    parser.add_argument('--profile', type=str, default='default', help='Authentication profile (default: default)')
    ...
    # Initialize manager with profile
    auth = AuthManager(profile_name=args.profile)

    # Execute command
    if args.command == 'setup':
        if auth.setup_auth(headless=args.headless, timeout_minutes=args.timeout):
            print("\n✅ Authentication setup complete!")
            print(f"You can now use ask_question.py --profile {args.profile} to query NotebookLM")
        else:
        ...
        print(f"\n🔐 Authentication Status (Profile: {args.profile}):")
